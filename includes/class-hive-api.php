...
class WP_Dapp_Hive_API {
	...
	public function prepare_post_data( $post_data ) {
		...
		$tags = isset( $post_data['tags'] ) ? array_map( 'sanitize_text_field', $post_data['tags'] ) : array();
		if ( empty( $tags ) ) {
		...
		return array(
			...
			'image'         => isset( $post_data['featured_image'] ) ? array( $post_data['featured_image'] ) : array(),
		);
	}
	...
	public function get_content( $author, $permlink ) {
		...
		$result = $this->rpc_call( 'condenser_api.get_content', array( $author, $permlink ) );
		return $result;
	}
	...
	public function get_content_replies( $author, $permlink ) {
		...
		$result = $this->rpc_call( 'condenser_api.get_content_replies', array( $author, $permlink ) );
		return $result;
	}
