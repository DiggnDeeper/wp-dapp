...
class WP_Dapp_Frontend {
	...
	public function __construct() {
		...
		add_shortcode( 'wpdapp_hive_comments', array( $this, 'render_hive_comments_shortcode' ) );
		add_filter( 'the_content', array( $this, 'append_notice_when_comments_closed' ) );
		add_filter( 'comments_open', array( $this, 'maybe_force_comments_closed' ), 20, 2 );
		add_filter( 'comments_template', array( $this, 'maybe_replace_comments_template' ), 20 );
	}
	 ...
	 */
	public function render_hive_comments_shortcode( $atts = array() ) {
		$atts = shortcode_atts(
			array(
				'post_id'          => 0,
				'show_reply_links' => '1',
			...
			if ( $hive_only_display ) {
				...
				if ( ! is_wp_error( $replies ) && ! empty( $replies ) ) {
					...
					foreach ( $replies as $reply ) {
						...
						if ( ! isset( $by_parent_key[ $parent_key ] ) ) {
							$by_parent_key[ $parent_key ] = array(); }
	...
	private function render_comment_branch( $by_parent, $parent_id, $thread_url_base, $depth = 0, $max_depth = 4 ) {
		...
		foreach ( $by_parent[ $parent_id ] as $c ) {
			...
			} elseif ( ! empty( $by_parent[ intval( $c->comment_ID ) ] ) ) {
	...
	private function render_hive_branch( $by_parent_key, $parent_key, $thread_url_base, $depth = 0, $max_depth = 4 ) {
		...
		foreach ( $by_parent_key[ $parent_key ] as $reply ) {
			...
			} elseif ( ! empty( $by_parent_key[ $key ] ) ) {
				$html .= '<div class="wpdapp-show-more"><a href="' . esc_url( $thread_url_base ) . '" target="_blank" rel="noopener nofollow">' . esc_html__( 'View more replies on Hive', 'wp-dapp' ) . '</a></div>';
			}
			$html .= '</li>';
		}
